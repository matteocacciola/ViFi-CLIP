...
def parse_option():
    ...
    parser.add_argument('--experiment-name', type=str, default='ViFi-CLIP_Few-Shot', help='Name of the MLflow experiment')
    parser.add_argument('--run-name', type=str, default=None, help='Name of the MLflow run')
...
def main_mlflow():
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        # opt_level removed since AMP is disabled
        ...
        model, criterion, optimizer, lr_scheduler, train_loader, val_loader, mixup_fn, start_epoch, max_accuracy, class_names = res_setup

        for epoch in range(start_epoch, config.TRAIN.EPOCHS):
            train_loader.sampler.set_epoch(epoch)
            train_loss = train_one_epoch(epoch, model, criterion, optimizer, lr_scheduler, train_loader, mixup_fn)
            ...
            if epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1):
                ...
                if dist.get_rank() == 0:
                    ...
                    # calculating only the classes in y_true or y_pred
                ...
                model_epoch_saving(epoch, model, optimizer, lr_scheduler, acc1, precision, recall, f1, max_accuracy)
...
def main():
    ...
    for epoch in range(start_epoch, config.TRAIN.EPOCHS):
        train_loader.sampler.set_epoch(epoch)
        train_one_epoch(epoch, model, criterion, optimizer, lr_scheduler, train_loader, mixup_fn)
...
def setup():
    ...
    return model, criterion, optimizer, lr_scheduler, train_loader, val_loader, mixup_fn, start_epoch, max_accuracy, class_names


def model_epoch_saving(epoch, model, optimizer, lr_scheduler, acc1, precision, recall, f1, max_accuracy):
    logger.info(
...
def multiview_inference(model):
    # the multi-view inference crop for videos
...
def train_one_epoch(epoch, model, criterion, optimizer, lr_scheduler, train_loader, mixup_fn):
    model.train()
    optimizer.zero_grad()
